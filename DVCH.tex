...
\midrule
$\Omega_m$ & 0.2953 & $0.2777-0.3140$ & $0.2598-0.3293$ \\
$n$ & 0.2164 & $0.0829-0.3480$ & $0.0155-0.4764$ \\
$\beta$ & 0.4824 & $0.1565-0.8285$ & $0.0300-0.9766$ \\
$H_0$ & 68.38 & $67.24-69.48$ & $66.14-70.45$ \\
\bottomrule
\end{tabular}
\end{minipage}
\end{center}
...
\begin{itemize}
    \item Maximum Gelman-Rubin statistic: $\max(\widehat R) = 1.0938$ ($\beta$; $\Omega_m$ 1.0451, $n$ 1.0421, $H_0$ 1.0583)
    \item Effective sample sizes: $N_{\rm eff} = [403, 458, 306, 551]$ for $[\Omega_m, n, \beta, H_0]$
    \item Minimum effective sample size: $\min(N_{\rm eff}) = 306$ ($\beta$)
    \item Acceptance fraction: 0.497
\end{itemize}

These values do \emph{not} satisfy the adopted $\widehat R < 1.01$ criterion, so this chain is treated as a local late-time diagnostic rather than a fully converged production posterior. The evidence values recorded for this run are $\chi^2_{\rm best}=28.03$ for $n_{\rm data}=43$ and $k=4$ parameters, giving $\mathrm{AIC}=36.03$ and $\mathrm{BIC}=43.07$ (\texttt{dvch\_mcmc\_full\_evidence.csv}).
...
\begin{minipage}{0.96\linewidth}\footnotesize
\textbf{Figure~\thefigure.} MCMC trace plots showing the evolution of the four sampled parameters across the chain iterations. The $\Omega_m$ and $H_0$ traces are stationary after burn-in, whereas the $\beta$ trace shows a slow drift that is not fully mixed by the end of the chain --- consistent with its $\widehat R = 1.0938$ and low ESS, and the reason this run is treated as a diagnostic rather than a converged posterior.
\end{minipage}
\end{center}

Model comparison statistics for our analysis yield:
\begin{itemize}
    \item Best-fit chi-squared: $\chi^2 = 28.03$ (43 data points, $k=4$ parameters)
