...
\newcommand{\sigeightref}{\sigma_{8,\Lambda\mathrm{CDM}}}
\newcommand{\kmsmpc}{\mathrm{km\,s^{-1}\,Mpc^{-1}}}
\newcommand{\Cl}[1]{C_\ell^{#1}}
\newcommand{\Clphi}{C_L^{\phi\phi}}
\newcommand{\calL}[1]{\mathcal{L}_{\mathrm{#1}}}
\newcommand{\lnL}[1]{\ln\calL{#1}}
\newcommand{\chisq}[1]{\chi^2_{\mathrm{#1}}}
% Distance modulus for argument #1
\newcommand{\distmod}[1]{5\log_{10}\!\left[\frac{D_L(#1)}{\mathrm{Mpc}}\right]+25}
% Gaussian data-vector likelihood block, identical for BAO/SNe/RSD
\newcommand{\gausschisq}[1]{%
\chisq{#1}=\Delta_{\mathrm{#1}}^{T}C_{\mathrm{#1}}^{-1}\Delta_{\mathrm{#1}},\qquad
\lnL{#1}=-\frac{1}{2}\chisq{#1}}

% ---- Text-mode shorthands ------------------------------------
\newcommand{\lcdm}{$\Lambda$CDM\xspace}
% File/module names: underscores are typed literally
\DeclareRobustCommand{\file}[1]{\texttt{\detokenize{#1}}}

% ---- Manuscript metadata (single source of truth) -------------
\newcommand{\authorname}{Daniel Castro Pasten Benjamin}
\newcommand{\authoremail}{daniel.castropasten6@gmail.com}
\newcommand{\authoraffiliation}{Theoretical Cosmology / High Energy Physics Unit}
\newcommand{\acknowledgementtext}{The author acknowledges technical support and
computational resources from the \authoraffiliation.}
% Output files of the modified linear Boltzmann solver
\newcommand{\boltzmannoutputfiles}{%
\file{dvch_boltzmann_modified_background.csv},
\file{dvch_boltzmann_modified_growth.csv},
\file{dvch_boltzmann_modified_matter_power.csv}, and
\file{dvch_boltzmann_modified_summary.csv}}

% ==============================================================
%  Shared float/table builders
% ==============================================================
% Full-width figure: #1 file (in figures/), #2 caption, #3 label
\newcommand{\widefigure}[3]{%
\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/#1}
\caption{#2}
\label{#3}
\end{figure*}}

% Non-floating status figure: #1 label, #2 file, #3 width fraction, #4 caption
\newcommand{\statusfigure}[4]{%
\begin{center}
\refstepcounter{figure}\label{#1}%
\includegraphics[width=#3\linewidth]{figures/#2}
\begin{minipage}{0.96\linewidth}\footnotesize
\textbf{Figure~\thefigure.} #4
\end{minipage}
\end{center}}

% Floating diagnostic table: #1 label, #2 caption, #3 size, #4 colspec, #5 header
\newenvironment{diagtable}[5]{%
\begin{table*}[t]
\centering
\caption{#2}
\label{#1}
...
% Non-floating status table: #1 label, #2 title, #3 note, #4 colspec, #5 header
\newenvironment{statustable}[5]{%
\begin{center}
...
The standard \lcdm model, while remarkably successful, is under intense
...
\file{clik} likelihood configuration target \cite{Planck:2019nip}, and a
...
whose comparison with \lcdm requires the likelihood analyses described
...
reported through $\fsig$ and the derived combination
...
history, but a quantitative comparison with \lcdm requires CMB-calibrated
...
diagnostic implementation, \file{DVCH_easy_colab.ipynb}, accompanied by the plain Python
script \file{dvch_colab_simple.py}. This route requires only standard
Python packages, evaluates the implicit DVCH background, writes
\file{dvch_background_table.csv}, and produces the diagnostic summary figure
shown below. The second level consists of the more advanced Cobaya diagnostic implementation
\file{dvch_theory.py}+\file{dvch_cobaya.yaml}, intended as the
...
\widefigure{dvch_results_with_numbers.png}{Complete easy DVCH diagnostic
output with plots and numerical
values. The top row shows $H(z)$, the density components, and the interaction
kernel $\Qtilde=Q/(3H_0\Ecrit)$. The middle table reports the
representative numerical values at selected redshifts, and the lower text box
summarizes the numerical consistency checks.}{fig:dvch_easy_summary}




\widefigure{dvch_sigma8_global_comparison.png}{Growth, $\sigma_8/S_8$, and
global comparison dashboard for the
...
$\sigeightref=0.811$, not a substitute for a final
Planck-level Boltzmann posterior. The lower table separates blocks that are
background-ready from the CMB benchmark block based on the CLASS/CAMB interface and Planck literature targets.}{fig:dvch_sigma8_global}

\widefigure{dvch_realdata_diagnostics.png}{Additional non-overlapping
real-data best-fit diagnostics generated
...
Fig.~\ref{fig:dvch_sigma8_global} is intentionally omitted here.}{fig:dvch_realdata_diagnostics}

\begin{diagtable}{tab:realdata_diagnostics}{Non-overlapping diagnostic numbers
associated with
...
in Fig.~\ref{fig:dvch_sigma8_global} and Table~\ref{tab:final_global_status}.}{}{lc}{Diagnostic & Value}
DVCH transition redshift $z_t$ from $q(z_t)=0$ & $0.70755$ \\
\lcdm reference transition redshift $z_t$ & $0.64263$ \\
Transition shift $\Delta z_t$ & $+0.06492$ \\
$\min\,\Qtilde$ over $-0.95\le z\le2$ & diagnostic output \\
$\max\,\Qtilde$ over $-0.95\le z\le2$ & diagnostic output \\
\end{diagtable}


\widefigure{dvch_robustness_scan.png}{Parameter-space robustness scan over
$0.05\le n\le0.45$ and
...
the background closure is numerically robust beyond a single fiducial point.}{fig:dvch_robustness_scan}

\begin{diagtable}{tab:dvch_robustness_scan}{Summary of the DVCH robustness
scan. The scan is deliberately limited
to background-level viability and interaction-sign consistency; full
perturbative/CMB viability remains part of the validation checklist.}{}{lc}{Diagnostic & Result}
...
Largest $\Qtilde$ among viable points & $-5.23\times10^{-3}$ \\
\end{diagtable}

\begin{diagtable}{tab:final_global_status}{Current DVCH--\lcdm diagnostic
status using the files
...
\cite{Planck:2019nip,Blas:2011rf,Lewis:1999bs,Torrado:2020dgo}.}{\scriptsize}{p{0.20\textwidth}p{0.21\textwidth}p{0.24\textwidth}p{0.25\textwidth}}{Block & Local input & Local output/status & Interpretation}
Background $H(z)$ & \file{dvch_background_table.csv} and
\file{dvch_robustness_scan.csv} &
...
Interaction $Q$ & \file{dvch_background_table.csv} &
$\Qtilde(0)=-0.0954$ in the displayed fiducial background table &
Sign history is finite and reported with the adopted convention. \\
Growth $\sigma_8/S_8$ & \file{dvch_growth_sigma8_table.csv} and
\file{dvch_sigma8_s8_summary.csv} &
$\sigma_{8,\rm DVCH}\simeq S_{8,\rm DVCH}\simeq0.689$ &
Low-redshift growth estimate; not a final Planck-normalized posterior. \\
Pantheon$+$ SNe & \file{dvch_joint_realdata_fit.py} &
...
DESI BAO/RSD & \file{dvch_joint_realdata_fit.py} &
...
Cosmic chronometers & \file{dvch_joint_realdata_fit.py} &
...
Information criteria & \file{joint-fit summary CSV} &
...
these statuses are summarized in \file{dvch_execution_validation_summary.csv}.
The background diagnostic gives $E^2_{\min}=1.0$, non-negative densities,
$\max\Qtilde=-0.095446<0$ over $0\le z\le5$, and a transition redshift
...
Boltzmann diagnostic reports $\fsig(z=0)=0.364139$ and
$S_8=0.829$, while explicitly flagging
\file{no_full_photon_baryon_hierarchy}. The full relativistic validation
gate finds the local DVCH source interface and Planck YAML target, but the
patched CLASS/CAMB backend, Planck \file{clik}, and local Planck data are
...
\widefigure{dvch_late_time_residuals.png}{Late-time residual diagnostic
generated from the local exploratory
fit. The panels compare DVCH and \lcdm residuals for compressed DESI BAO
and cosmic-chronometer points available to the diagnostic implementation \cite{Adame:2024ywp,Moresco:2016mzx}. This is not a CMB residual plot; Planck TT/TE/EE information is used here only through published benchmark constraints \cite{Planck:2019nip}.}{fig:dvch_late_time_residuals}
...
estimate. Normalizing to a reference \lcdm value
$\sigeightref=0.811$, the fiducial DVCH background gives
\begin{equation}
...
\end{aligned}
\end{equation}
where the differences are defined as DVCH minus \lcdm for the exploratory
...
\lcdm. The rest of this manuscript therefore makes those completion
criteria explicit.
...
implied by DVCH. For the sub-horizon modes used in $\fsig$ analyses,
...
\subsection{Sub-horizon growth equation used for \texorpdfstring{$\fsig$}{f sigma 8}}
\label{subsec:growth}
...
to \lcdm depending on epoch and parameter values. This interaction
...
reference value $\sigeightref=0.811$ unless explicitly noted
...
sub-horizon limit used for $\fsig$ is identified as a controlled
...
\file{dvch_relativistic_perturbation_validation.py}. It records whether the
...
\begin{statustable}{tab:relativistic_perturbation_validation}{Relativistic
perturbation preflight/status gate for DVCH.}{``Available'' means the local
interface file or configuration target is present;
``Protocol target'' means the component is specified as a benchmark requirement
rather than executed as a new Planck posterior calculation.}{p{0.40\linewidth}p{0.18\linewidth}p{0.30\linewidth}}{Validation item & Status here & Requirement}
...
\statusfigure{fig:relativistic_perturbation_validation}{dvch_relativistic_perturbation_validation_status.png}{0.96}{Executable
relativistic-perturbation preflight/status
...
\end{equation}
Common choices are $X=\{H(z),D_M(z),\fsig(z)\}$ for BAO+RSD forecasts in
...
with, e.g.\ SNe Ia
\begin{equation}
\chisq{SN} = \Delta\bm{\mu}^{T}\,C_{\mathrm{SN}}^{-1}\,\Delta\bm{\mu},
\end{equation}
...
When comparing DVCH with \lcdm, information criteria must be reported
...
accurate statement is that DVCH is not preferred over \lcdm by BIC even
...
super-horizon perturbations, finite $\fsig(z)$, and a stable
...
\file{DVCH_easy_colab.ipynb} or \file{python dvch_colab_simple.py} to
reproduce the background table and figures; (ii) use the generated CSV file to
inspect $E(z)$, $H(z)$, $\Omega_m(z)$, $\Omega_\Lambda(z)$, and
$\Qtilde(z)$; and (iii) only after this background validation, move to the
Cobaya diagnostic implementation for likelihood-level work. The correct \file{Cobaya}
interface for the background module is to define the hooks
\file{initialize()}, \file{get_can_provide()},
\file{must_provide()}, \file{calculate()}, and
\file{get_result()}, rather than ad hoc methods such as
\file{get_can_support_params()} or \file{get_distances()} that are
...
\file{dvch_modified_boltzmann_solver.py}. This code is separate from the
...
source, computes the normalized linear growth $D(z)$ and $\fsig(z)$, and
exports a matter transfer/power-spectrum diagnostic. The generated files are
\boltzmannoutputfiles, and the figure
\file{figures/dvch_modified_boltzmann_solver.png}. For the fiducial run included
...
\statusfigure{fig:dvch_modified_boltzmann_solver}{dvch_modified_boltzmann_solver.png}{0.96}{Executable
DVCH modified linear Boltzmann-diagnostic implementation output. The left
panel shows the normalized matter growth factor and $\fsig(z)$ obtained by
...
\begin{itemize}
    ...
    \item the Planck \file{clik} likelihood evaluation.
\end{itemize}
...
machine where the external Planck \file{clik} data and a patched compiled
...
\textbf{CMB/CLASS-CAMB status.} The package includes a CLASS/CAMB--Planck interface layer: the executable modified linear solver for the DVCH background and matter perturbations, the DVCH source-table interface for a compiled backend, the Planck 2018 target configuration, and benchmark targets for the photon--baryon hierarchy, line-of-sight sources, angular spectra $C_\ell$, CMB lensing, and the Planck \file{clik} likelihood. In this article those components define only the CMB interface/protocol; numerical Planck likelihood values are taken only from the published Planck literature.
\end{minipage}}
\end{center}
...
The archive includes \file{dvch_cmb_class_camb_mcmc.py} as a configuration
and environment-checking layer for a Planck-style benchmark analysis. It records
whether Cobaya, a CLASS or CAMB Python backend, the Planck \file{clik}
...
\statusfigure{fig:dvch_cmb_class_camb_mcmc_status}{dvch_cmb_class_camb_mcmc_status.png}{0.88}{Planck-style
benchmark-interface diagnostic for the
...
is generated by \file{dvch_cmb_class_camb_mcmc.py} and is not a Planck
posterior-convergence plot.}

\subsection{Executable MCMC convergence diagnostic}
\label{subsec:mcmc_convergence}

As a local sampler check, the archive includes \file{dvch_mcmc_convergence.py}.
The script runs four independent Metropolis chains for the local DVCH
BAO+chronometer likelihood, writes the chains to \file{dvch_mcmc_chains.csv},
and reports posterior summaries in \file{dvch_mcmc_convergence_summary.csv}.
For the run included here, the convergence diagnostics are
\begin{equation}
\max(\widehat R)=1.093,\qquad \min(N_{\rm eff})\simeq 36,
\end{equation}
with chain acceptance fractions stored in \file{dvch_mcmc_acceptance.csv}.
...
\statusfigure{fig:dvch_mcmc_convergence}{dvch_mcmc_convergence.png}{0.96}{Executable
DVCH MCMC convergence diagnostic for the local
BAO+chronometer likelihood. The dashed line marks burn-in. The displayed chains
are generated by \file{dvch_mcmc_convergence.py} and summarized by
\file{dvch_mcmc_convergence_summary.csv}. This figure documents a
...
\begin{statustable}{tab:final_dvch_posterior}{Current DVCH posterior/diagnostic
summary available in this archive.}{The values are those available from the
present late-time diagnostic analysis;
they should not be reinterpreted as a completed Planck CMB posterior.}{lll}{Quantity & Value & Status}
$H_0$ & $69.03~\kmsmpc$ & late-time best fit \\
...
Cobaya configuration, \file{dvch_planck2018_full.yaml}, a local
environment checker, \file{dvch_planck_preflight.py}, and the DVCH
Boltzmann source module \file{dvch_boltzmann_backend.py}, and the
executable local modified linear solver
\file{dvch_modified_boltzmann_solver.py}. These files specify the exact
...
\lcdm's eternal de Sitter. This has implications for:
...
dynamics and the sub-horizon $\fsig$ growth framework, the supplied
\file{dvch_modified_boltzmann_solver.py} executes a modified linear
Boltzmann diagnostic implementation for the DVCH background and matter perturbations, while
\file{dvch_boltzmann_backend.py} supplies the source-table interface for a future
...
$C_\ell$, CMB lensing, Planck \file{clik}, and Planck-style MCMC runner as
...
eternal de Sitter fate of \lcdm. This reframes the cosmological-constant
...
self-contained files \file{DVCH_easy_colab.ipynb},
\file{dvch_colab_simple.py}, \file{dvch_background_table.csv}, \file{dvch_execution_validation_summary.csv}, the
figures in \file{figures/}, including \file{dvch_results_with_numbers.png},
\file{dvch_sigma8_global_comparison.png}, and
\file{dvch_realdata_diagnostics.png}, and
\file{dvch_robustness_scan.png}, the robustness outputs
\file{dvch_robustness_scan.csv} and
\file{dvch_robustness_summary.csv}, the growth summaries
\file{dvch_growth_sigma8_table.csv} and
\file{dvch_sigma8_s8_summary.csv}, the exploratory public-data joint-fit
diagnostic implementation \file{dvch_joint_realdata_fit.py} with summary
\file{dvch_joint_realdata_fit_summary.csv}, and the advanced Cobaya
diagnostic implementation \file{dvch_theory.py}+\file{dvch_cobaya.yaml}, together
with the Planck preflight target \file{dvch_planck2018_full.yaml},
checker \file{dvch_planck_preflight.py}, Boltzmann-source module
\file{dvch_boltzmann_backend.py}, and integration notes
\file{dvch_class_integration_notes.md}, the relativistic perturbation preflight/status script
\file{dvch_relativistic_perturbation_validation.py} with JSON/CSV/PNG
status outputs, the executable modified linear solver
\file{dvch_modified_boltzmann_solver.py}, and its outputs
\boltzmannoutputfiles, and the figure
\file{figures/dvch_modified_boltzmann_solver.png}.
...
$\Qtilde \equiv Q/(3H_0\Ecrit)$ becomes
\begin{equation}
    \Qtilde(z) = -\frac{E\,\Omega_\Lambda}{1 + n\frac{\Omega_\Lambda}{\Omega_m}} \left[ n - \frac{\beta}{1+\beta E^2} \left( \frac{4\Omega_r + 3\Omega_m}{3} \right) \right]
\end{equation}
...
implementation checklist for a standard spatially flat \lcdm run and as
...
\begin{enumerate}
    \item Fix the baseline cosmological model: spatially flat \lcdm with six primary parameters and standard radiation/neutrino assumptions.
    \item Define priors for all cosmological, calibration, and nuisance parameters.
    \item Select likelihood modules: Planck 2018 TT,TE,EE+lowE+lensing \cite{Aghanim:2018eyx,Planck:2019nip}; BAO \cite{Beutler:2011hx,Ross:2014qpa,Alam:2016hwk,Alam:2020sor}; Pantheon/Pantheon+-type SNe \cite{Brout2022}; and RSD $\fsig$ measurements \cite{Alam:2016hwk,Alam:2020sor}.
...
encodes the clustering fit. RSD compressed analyses use $\fsig(z)$ and its
...
theory vector contains the lensed CMB spectra $\Cl{TT}$, $\Cl{TE}$,
$\Cl{EE}$ and the lensing-potential spectrum $\Clphi$. The official
likelihood is not a single elementary Gaussian in all multipoles; in practice it
is called through the Planck \file{clik} interface or a Cobaya/MontePython
wrapper,
\begin{equation}
\lnL{Planck}=\lnL{clik}\left(\Cl{TT},\Cl{TE},\Cl{EE},\Clphi,\theta_{\rm nuis}^{\rm Planck}\right).
\end{equation}
...
\end{equation}
For a fixed published covariance $C_{\mathrm{BAO}}$,
\begin{equation}
\gausschisq{BAO},
\end{equation}
with $\Delta_{\mathrm{BAO}}=d_{\rm BAO}^{\rm obs}-d_{\rm BAO}^{\rm th}(\theta)$.
...
\begin{equation}
\mu_{\mathrm{th}}(z_i)=\distmod{z_i}+\Delta_M,
\end{equation}
...
For RSD, an initial robust implementation uses compressed $\fsig(z)$ points
with their covariance matrix. The theoretical prediction is
\begin{equation}
\fsig(z)=\frac{\dd\ln D}{\dd\ln a}\,\sigma_8(z),
\end{equation}
...
\lcdm analysis. For each proposed parameter vector, the solver must
...
\file{output=tCl,pCl,lCl,mPk}, \file{lensing=yes}, a sufficiently large
\file{l_max_scalars}, and a sufficiently large \file{P_k_max_h/Mpc}
...
\begin{equation}
\lnL{tot}=\lnL{Planck}+\lnL{BAO}
+\lnL{SNe}+\lnL{RSD}.
\end{equation}
The posterior is
\begin{equation}
\ln P(\theta|d)=\lnL{tot}(\theta)+\ln \pi(\theta),
\end{equation}
...
\begin{enumerate}
    ...
    \item Evaluate $\lnL{Planck}$, $\lnL{BAO}$, $\lnL{SNe}$, and $\lnL{RSD}$ for the active dataset combination.
...
\begin{enumerate}
    \item Compare $\Cl{TT}$, $\Cl{TE}$, $\Cl{EE}$, and $\Clphi$ against a reference CLASS/CAMB or Planck benchmark output.
    ...
    \item Confirm that $\sigma_8(z)$ and $\fsig(z)$ behave physically over the redshift range used by the RSD data.
\end{enumerate}
...
redshift; the SNe Hubble diagram and residuals; RSD $\fsig(z)$ with the
...
\emph{Data and Methods} section defines the \lcdm model, priors, Planck
...
and the DVCH Boltzmann source-table interface, but it does not contain the
external runtime components needed for a real Planck posterior:
\file{cobaya}, \file{classy} or \file{camb}, the Planck \file{clik}
likelihood module, \file{cobaya-run}, and a local Planck data directory.
