...
gate finds the local DVCH source interface and Planck YAML target. The patched
CAMB runtime has been compiled and the official Planck high-$\ell$
\texttt{plik\_rd12\_HM\_v22b\_TTTEEE.clik} data directory has been opened
successfully by \texttt{clik}; its built-in reference vector reproduces the
package check value. This validates the likelihood library/data connection
only. The CAMB-to-Cobaya spectrum vector, nuisance-parameter mapping, and
posterior sampling with the full nuisance sector remains unexecuted. A
12-accepted-step Cobaya smoke chain with the official fixed check vector has
been run successfully as an integration test; it is not a converged
cosmological posterior and is not reported as one.

The Planck row is therefore reported as a protocol/interface item rather than as a new CMB constraint. The local files are the DVCH source-table interface, the homogeneous CAMB-ready DVCH background provider, the linear synchronous-gauge perturbation closure, the CAMB Fortran interaction module, the Planck YAML target, the matter-sector modified linear solver, the validation-status files, and a non-redundant main-text figure set. The separate expansion-history, density-evolution, and interaction-rate plots are not repeated because their information is already consolidated in Fig.~\ref{fig:dvch_easy_summary}; the main text keeps the non-redundant composite and status figures only. Numerical Planck likelihood values, posterior contours, and Bayesian evidence are not quoted as DVCH results in this article; Planck is used as a published benchmark for interpreting the background and growth diagnostics \cite{Aghanim:2018eyx,Planck:2019nip,Jeffreys:1961}.
...
We implement a late-time MCMC pipeline (\texttt{dvch\_full\_mcmc\_pipeline.py}) using the \texttt{emcee} ensemble sampler to constrain the DVCH background parameters against the fast cosmic-chronometer (33 $H(z)$ points, Moresco et al.\ 2016 compilation) and DESI DR1 BAO (10 $D_M/r_d$ and $D_V/r_d$ points) dataset, $N_{\rm data}=43$; Pantheon$+$ supernovae are not included in this run. The analysis uses 24 walkers for 2000 steps with a burn-in of 800 steps; thinning by half of the shortest integrated autocorrelation time leaves 1104 stored samples (\texttt{dvch\_mcmc\_chains\_full.csv}).

The free parameters in this MCMC analysis, with flat priors, are:
\begin{itemize}
    \item $\Omm$: present matter density, $0.05<\Omm<0.95$
    \item $n$: DVCH tracking parameter, $0.001<n<0.999$
    \item $\beta$: DVCH curvature suppression parameter, $0\le\beta<1$
    \item $H_0$: Hubble constant, $40<H_0<120~\mathrm{km\,s^{-1}\,Mpc^{-1}}$
\end{itemize}
The sound horizon is held fixed at $r_d=147.09~\mathrm{Mpc}$; no supernova nuisance parameters enter this likelihood.
...
\centering
\footnotesize
\textbf{Table~\thetable. DVCH parameter posteriors from the late-time CC+DESI-BAO MCMC run.}
Posterior median and 68\%/95\% credible intervals, read from \texttt{dvch\_mcmc\_full\_summary.csv}.
The $\beta$ posterior fills its flat prior and is therefore unconstrained by this dataset.
\begin{tabular}{lccc}
\toprule
Parameter & Median & 68\% CI & 95\% CI \\
\midrule
$\Omm$ & 0.295 & $0.278-0.314$ & $0.260-0.329$ \\
$n$ & 0.216 & $0.083-0.348$ & $0.016-0.476$ \\
$\beta$ & 0.482 & $0.156-0.828$ & $0.030-0.977$ \\
$H_0~[\mathrm{km\,s^{-1}\,Mpc^{-1}}]$ & 68.38 & $67.24-69.48$ & $66.14-70.45$ \\
\bottomrule
\end{tabular}
\end{minipage}
\end{center}

Convergence diagnostics for this chain, as written by the pipeline to \texttt{dvch\_mcmc\_full\_convergence.csv} and \texttt{dvch\_mcmc\_full\_evidence.csv}, are (parameter order $\Omm,\,n,\,\beta,\,H_0$):
\begin{itemize}
    \item Split-chain Gelman-Rubin statistic: $\widehat R = [1.045,\,1.042,\,1.094,\,1.058]$, i.e.\ $\max(\widehat R) = 1.094$ (for $\beta$)
    \item Effective sample size: $N_{\rm eff} = [403,\,458,\,306,\,551]$, i.e.\ $\min(N_{\rm eff}) = 306$ (for $\beta$)
    \item Integrated autocorrelation times implied by $N_{\rm eff}=N_{\rm walkers}(N_{\rm steps}-N_{\rm burn})/\tau$: $\tau \simeq [71,\,63,\,94,\,52]$
    \item Mean acceptance fraction: 0.497
\end{itemize}

These values do \emph{not} satisfy the $\widehat R-1<0.01$ threshold adopted for a production posterior; all four parameters exceed $\widehat R=1.04$, and $\beta$ and $H_0$ also exceed the looser $\widehat R<1.05$ screening value. The minimum ESS is likewise below the preferred screening value of 400. The chain is therefore reported only as a local late-time sampler diagnostic: the intervals in Table~\ref{tab:dvch_mcmc_posteriors} are indicative and must not be quoted as converged constraints. Substantially longer chains (or more walkers) are required before this analysis can be promoted to a production posterior.
...
\begin{minipage}{0.96\linewidth}\footnotesize
\textbf{Figure~\thefigure.} MCMC trace plots showing the evolution of parameter values across the chain iterations for the CC+DESI-BAO run. The residual drifts visible here are consistent with the $\widehat R>1.04$ values reported in the text; the run is a sampler diagnostic, not a converged posterior.
\end{minipage}
\end{center}

Model comparison statistics for this run (\texttt{dvch\_mcmc\_full\_evidence.csv}) yield:
\begin{itemize}
    \item Best-fit chi-squared: $\chi^2 = 28.03$ (43 data points, 4 free parameters)
    \item Akaike Information Criterion: $\mathrm{AIC} = 36.03$
    \item Bayesian Information Criterion: $\mathrm{BIC} = 43.07$
\end{itemize}
...
matter-sector solver, and the late-time MCMC diagnostics. In WSL2 Ubuntu, the
DVCH CAMB fork was compiled with \texttt{gfortran}; a smoke test produced
finite scalar spectra with \texttt{DVCH\_flag=True} and a distinct LCDM
comparison. The Planck \texttt{clik} source also produced a resolved shared
library. The extracted Commander TT and lensing likelihoods are connected to
the Cobaya adapter; the SimAll EE binary is present but cannot be loaded by
the compiled clik library because its \texttt{simall} plugin symbol is absent.
Consequently the local tests and late-time MCMC diagnostics are complete for
their implemented scope. The official high-$\ell$ TTTEEE \texttt{.clik}
likelihood is now connected to Cobaya, and a reproducible 32-sample
integration chain completed with Commander TT and lensing active. The chain
uses the official recommended Gaussian priors for the published dust and
temperature-calibration nuisance parameters; nuisance fields without a
published prior remain fixed at the official check point. Two independent
short chains were also evaluated, but their diagnostics have
$\hat R>1.2$ and ESS below 12 for several parameters. They are validation
artifacts, not a converged posterior.
...
be read as Planck-calibrated posterior constraints or evidence ratios. A publication-grade result still requires loading the SimAll EE plugin or
using the official Cobaya native low-$\ell$ EE implementation, substantially
longer independent chains, and convergence thresholds such as
$\hat R-1<0.01$ and adequate ESS.

The supplied Planck likelihood code and baseline likelihood archive were
inspected in the external workspace. The WSL2 build now provides the required
C/Fortran, BLAS/LAPACK, CFITSIO, and MPI toolchain, and produces
\texttt{libclik.so}. CAMB 2.0.4 and Cobaya 3.6.2 were verified locally,
including a baseline CAMB calculation with
$\sigma_8=0.80975$ at $H_0=67~\mathrm{km\,s^{-1}\,Mpc^{-1}}$ and a compiled
DVCH spectrum smoke test. The automatically generated broad nuisance ranges are deliberately not used:
they caused a zero-acceptance chain and are not official Planck priors.
