...
We implement a full late-time MCMC pipeline using the \texttt{emcee} ensemble sampler to constrain the DVCH parameter space with DESI DR1 BAO and cosmic chronometer data (43 data points in total). The analysis uses 20 walkers for 26000 steps with a burn-in of 5000 steps, producing 12000 effective samples after thinning (thin = 35).

The free parameters in our MCMC analysis are:
\begin{itemize}
    \item $\Omega_m$: present matter density fraction
...
Posterior median and 68\%/95\% credible intervals (from \texttt{dvch\_mcmc\_full\_summary.csv}).
\begin{tabular}{lccc}
\toprule
Parameter & Median & 68\% CI & 95\% CI \\
\midrule
$\Omega_m$ & 0.2949 & $0.2761-0.3130$ & $0.2563-0.3303$ \\
$n$ & 0.2173 & $0.0923-0.3619$ & $0.0209-0.4987$ \\
$\beta$ & 0.4862 & $0.1526-0.8344$ & $0.0228-0.9732$ \\
$H_0$ & 68.32 & $67.17-69.41$ & $66.07-70.41$ \\
\bottomrule
\end{tabular}
\end{minipage}
\end{center}

The reported convergence diagnostics (from \texttt{dvch\_mcmc\_full\_convergence.csv}) are:
\begin{itemize}
    \item Maximum Gelman-Rubin statistic: $\max(\widehat R) = 1.0048$ ($\beta$; $\Omega_m$ 1.0023, $n$ 1.0019, $H_0$ 1.0016)
    \item Effective sample sizes: $N_{\rm eff} = [5173, 5394, 3430, 5886]$ for $[\Omega_m, n, \beta, H_0]$
    \item Minimum effective sample size: $\min(N_{\rm eff}) = 3430$ ($\beta$)
    \item Acceptance fraction: 0.4927
\end{itemize}

These values satisfy the adopted convergence criteria, $\widehat R < 1.01$ and $N_{\rm eff} > 100$, for all four sampled parameters: the worst-mixed parameter is $\beta$, with $\widehat R = 1.0048$ and $N_{\rm eff} = 3430$, so the extended chain is treated as a converged late-time posterior rather than a merely local diagnostic. We emphasize that this is a background-only late-time fit (DESI DR1 BAO + cosmic chronometers, $n_{\rm data}=43$); it is not a full CMB analysis, and the DVCH-specific parameters $n$ and $\beta$ remain only weakly constrained by these data, as reflected in the broad credible intervals of Table~\ref{tab:dvch_mcmc_posteriors}. The evidence values recorded for this run are $\chi^2_{\rm best}=28.07$ for $n_{\rm data}=43$ and $k=4$ parameters, giving $\mathrm{AIC}=36.07$ and $\mathrm{BIC}=43.11$ (\texttt{dvch\_mcmc\_full\_evidence.csv}).
...
\begin{minipage}{0.96\linewidth}\footnotesize
\textbf{Figure~\thefigure.} Corner plot showing posterior distributions and correlations between the four sampled DVCH parameters ($\Omega_m$, $n$, $\beta$, $H_0$) from the full late-time MCMC analysis (DESI DR1 BAO + cosmic chronometers). Diagonal panels show marginalized posteriors, while off-diagonal panels show pairwise correlations.
\end{minipage}
\end{center}
...
\begin{minipage}{0.96\linewidth}\footnotesize
\textbf{Figure~\thefigure.} MCMC trace plots showing the evolution of the four sampled parameters across the chain iterations. The $\Omega_m$ and $H_0$ traces are stationary after burn-in, and the $\beta$ and $n$ traces, which were the slowest-mixing parameters in the shorter exploratory run, are now well mixed over the extended chain, with $\beta$ reaching $\widehat R = 1.0048$ and ESS $= 3430$ and $n$ reaching $\widehat R = 1.0019$ and ESS $= 5394$ --- consistent with all four parameters satisfying the adopted $\widehat R < 1.01$ and ESS $> 100$ criteria.
\end{minipage}
\end{center}

Model comparison statistics for our analysis yield:
\begin{itemize}
    \item Best-fit chi-squared: $\chi^2 = 28.07$ (43 data points, $k=4$ parameters)
    \item Akaike Information Criterion: $\mathrm{AIC} = 36.07$
    \item Bayesian Information Criterion: $\mathrm{BIC} = 43.11$
\end{itemize}
...
\begin{equation}
\max(\widehat R)=1.104,\qquad \min(N_{\rm eff})\simeq 57,
\end{equation}
...
\centering
\footnotesize
\textbf{Table~\thetable. Current DVCH best-fit/diagnostic summary available in this archive.}
These are point values from the joint real-data best fit (\texttt{dvch\_joint\_realdata\_fit.py}), not the posterior medians of the late-time MCMC chain of Table~\ref{tab:dvch_mcmc_posteriors} (where $n$ and $H_0$ are sampled and constrained by BAO + cosmic chronometers);
